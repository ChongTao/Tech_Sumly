...
## 3.11 High Watermark 与 LEO

排查 Kafka 读写与副本同步问题时，经常会看到两个位置概念：

- LEO：Log End Offset，当前日志末尾位置
- High Watermark：已经被 ISR 足够确认、可安全对消费者可见的位置

可以粗略理解为：

- 生产者写入后，Leader 的 LEO 会先前进
- 副本跟上后，High Watermark 才会推进
- 普通消费者通常只能读到 High Watermark

这也是为什么“Leader 已经收到了消息”不等于“消费者立刻就一定能读到”。

## 3.12 消费堆积怎么排查

消费堆积不是一个单点问题，常见根因通常在这几类：

- 生产速度突然升高
- 消费者处理能力不足
- 下游数据库、缓存、HTTP 接口变慢
- Rebalance 频繁触发，导致消费暂停
- 热点 Partition 导致部分消费者很忙、部分消费者空闲

排查时通常按这个顺序看：

1. 堆积是否集中在少数 Topic 或少数 Partition
2. 消费者是否存活、是否反复重启
3. 单条消息处理耗时是否上升
4. 下游依赖是否变慢
5. 是否因为扩容不当导致分区不均或热点 Key

治理思路通常包括：

- 扩容 Consumer Group 实例
- 优化单条消息处理逻辑
- 把慢操作异步化或批处理化
- 拆分热点 Topic 或优化 Key 设计
- 对失败消息单独分流，避免拖垮主链路
