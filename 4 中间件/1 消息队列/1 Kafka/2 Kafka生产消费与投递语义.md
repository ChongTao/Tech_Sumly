...
Kafka 本身更像一个日志平台，不会像某些 MQ 一样默认帮你做好“失败自动重试队列”语义。实际项目里通常要自己设计重试链路。

常见做法：

- 短暂失败：消费者在应用内做有限次重试
- 可恢复失败：写入重试 Topic，稍后再次消费
- 不可恢复失败：写入死信 Topic，转人工排查或异步修复

设计时要注意：

- 重试消息最好带上重试次数、原始 Topic、错误原因
- 不要无限重试，否则会放大积压
- 重试 Topic 通常要和主 Topic 分开，避免正常消费链路被异常消息拖慢
- 死信 Topic 不是“丢弃区”，而是排障入口

## 2.14 顺序与重试的冲突

很多系统既想要“严格顺序”，又想要“失败后跳过继续”，这两者经常冲突。

如果同一 Partition 内要求严格顺序，那么某条消息处理失败后，后续消息通常不应直接越过去处理，否则业务顺序就被破坏了。

常见策略：

- 顺序要求高：失败后阻塞当前 Partition，等待重试成功或人工处理
- 顺序要求一般：把失败消息转移到重试链路，主消费继续推进
- 用业务幂等兜底：允许少量重复，再由下游去重

所以讨论 Kafka 顺序性时，不能只看“同 Key 落同 Partition”，还要同时看失败处理策略。
